...
SELECT 
    ...
    test_phase
FROM query_performance_results
ORDER BY test_timestamp DESC, query_type;
...
SELECT 
    ...
    test_phase,
    COUNT(*) as test_count,
    ROUND(AVG(execution_time_ms), 3) as avg_execution_ms,
    ROUND(MIN(execution_time_ms), 3) as min_execution_ms,
    ROUND(MAX(execution_time_ms), 3) as max_execution_ms,
    ROUND(STDDEV(execution_time_ms), 3) as stddev_ms
FROM query_performance_results
GROUP BY query_type, test_phase
ORDER BY query_type, test_phase;

-- RLS overhead analysis
WITH baseline AS (
    SELECT 
        REPLACE(query_type, '_rls', '') as base_type,
        AVG(execution_time_ms) as baseline_time
    FROM query_performance_results
    WHERE test_phase = 'baseline'
    GROUP BY REPLACE(query_type, '_rls', '')
),
rls AS (
    SELECT 
        query_type,
...
SELECT 
    b.base_type,
    ROUND(b.baseline_time, 3) as baseline_ms,
    ROUND(r.rls_time, 3) as rls_ms,
    ROUND(r.rls_time - b.baseline_time, 3) as overhead_ms,
    ROUND((r.rls_time - b.baseline_time) / b.baseline_time * 100, 2) as overhead_pct
FROM baseline b
JOIN rls r ON r.query_type = b.base_type || '_rls'
ORDER BY overhead_pct DESC;

-- Export to CSV (for reporting)
-- COPY (
--     SELECT * FROM query_performance_results
-- ) TO '/tmp/query_performance_results.csv' WITH CSV HEADER;
*/